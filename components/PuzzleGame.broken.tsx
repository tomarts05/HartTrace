...
export const PuzzleGame: React.FC = () => {
  ...
  const maxPenPoints = performanceMode === 'low' ? 8 : performanceMode === 'medium' ? 15 : 25;
  const penUpdateFrequency = performanceMode === 'low' ? 100 : performanceMode === 'medium' ? 66 : 33; // milliseconds between updates
    ...
    rafIdRef.current = requestAnimationFrame(() => {
      ...
      if (svgRef.current) {
        try {
          ...
          const now = performance.now();
          const shouldUpdateCursor = (now - lastPenUpdateRef.current) >= penUpdateFrequency;
          ...
          // Only add if point is significantly different from last point to reduce redundancy
          const lastPoint = penPathRef.current[penPathRef.current.length - 1];
          const distance = lastPoint ? Math.sqrt(Math.pow(svgX - lastPoint.x, 2) + Math.pow(svgY - lastPoint.y, 2)) : Infinity;
          
          if (distance > 5) { // Only add points that are at least 5 units apart
            ...
            // Update pen path visualization with reduced frequency
            if (shouldUpdateCursor) {
