...
export const PuzzleGame: React.FC = () => {
  ...
  const handleInteractionMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    // DEBUG: Log movement for troubleshooting
    console.log('🖱️ Move event:', e.type, 'Drawing:', isDrawing, 'Point:', { x: point.clientX, y: point.clientY });
    ...
    if (!isAdjacent) {
      console.log('Cell not added - not adjacent (horizontal/vertical):', cell, 'from', lastCellInPath);
      return; // Don't add non-adjacent cells, but continue showing cursor line
    }

    // Rule 5: Path cannot cross itself
    if (currentPath.includes(cell)) {
      console.log('Cell not added - already in current path:', cell);
      return;
    }
    ...
    if (isOccupiedByCompletedPath) {
      console.log('Cell not added - occupied by completed path:', cell);
      return;
    }
    ...
    console.log('Added cell to path:', cell, 'Path length now:', newCurrentPath.length);
